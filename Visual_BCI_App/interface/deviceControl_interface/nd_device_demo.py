...
if sys.version_info[:2] != (3, 9):
    raise ImportError(
        "neuro_dance.core requires Python 3.9 because core.pyd imports "
        "python39.dll. Run this app with the bci_env/conda Python 3.9 "
        "interpreter instead of the current Python "
        f"{sys.version_info.major}.{sys.version_info.minor}."
    )

_PROJECT_ROOT = Path(__file__).resolve().parents[2]
_DEVICE_DLL_DIR = _PROJECT_ROOT / "source" / "device"
if _DEVICE_DLL_DIR.exists():
    if hasattr(os, "add_dll_directory"):
        os.add_dll_directory(str(_DEVICE_DLL_DIR))
    else:
        os.environ["PATH"] = str(_DEVICE_DLL_DIR) + os.pathsep + os.environ.get("PATH", "")
...
# 设备实际采样率（项目统一按 250Hz 运行）
DEVICE_EEG_SAMPLE_RATE = 250
# 存储5分钟的数据，计算存储的包数量
...
class NdDevice(NdDeviceBase):
    ...
    eeg_sample = DEVICE_EEG_SAMPLE_RATE  # 脑电图采样率
    mode = None  # 设备通信模式
    last_read_timestamp = 0  # 初始化为0
    
    def __init__(self, mode, com, tcp_ip, tcp_port, host_mac_bytes=None, eeg_sample=DEVICE_EEG_SAMPLE_RATE):
        """
        初始化设备
    ...
    def read_latest_eeg_data(self, target_freq=250):
        """
        读取自上次调用以来的全部新脑电图数据，并按照目标频率进行下采样
        :param target_freq: 目标采样频率（Hz），默认250Hz
        :return: 下采样后的脑电图数据，形状为(8,N)
        """
        # 设备实际采样率
        device_freq = self.eeg_sample

        # 计算下采样比率
        if target_freq <= 0:
            target_freq = device_freq
        if target_freq >= device_freq:
            downsample_ratio = 1
        else:
            downsample_ratio = max(1, int(round(device_freq / target_freq)))

        # 收集“上次读取之后”的全部新包，避免只取最新包导致中间包丢失。
        new_packets = []
        latest_time = self.last_read_timestamp
        for i in range(self.eeg_datas.length()):
            packet = self.eeg_datas.eleAt(i)
            if packet is None:
                continue
            ts = packet.get('timestamp', 0)
            if ts > self.last_read_timestamp:
        ...
        # 按时间顺序拼接，保持时序连续。
        new_packets.sort(key=lambda x: x.get('timestamp', 0))
        chunks = []
        for packet in new_packets:
            raw_data = packet.get('data', None)
            if raw_data is None:
                return None
            ...
            if isinstance(raw_data, np.ndarray):
                ...
                chunks.append(downsampled_data)
            else:
                # 对列表形式的数据进行下采样
                downsampled_data = []
                ...
                chunks.append(np.array(downsampled_data))
...
def tcp_test():
    ...
    """
    # 创建TCP模式的设备实例
    nd_device = NdDevice(mode='tcp', com='', tcp_ip='10.186.179.92', tcp_port=8899, host_mac_bytes=None, eeg_sample=DEVICE_EEG_SAMPLE_RATE)
