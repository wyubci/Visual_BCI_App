...
from interface.car_interface.ssvep_grid_canvas import (
    SSVEPGridCanvas,
    GridConfig as SSVEPGridCfg,
)
...
class CarControlWindow(QWidget):
    def __init__(self, objectName):
        ...
        self.stim_target_count = getattr(config, 'ssvep_grid_target_count', 40)
        self._spatial_mode = (self.stim_target_count >= 200)
        
        # 使用配置文件中的 40 频率列表 (8.0~15.8 Hz)
        self.sti_lst = [float(x) for x in config.sti_lst[:40]]
        self.stim_refresh_hz = _detect_screen_refresh_rate(default_hz=60.0)
        self.stim_period_frames = [int(round(self.stim_refresh_hz / f)) for f in self.sti_lst]
        self.stim_duty_frames = [max(1, p // 2) for p in self.stim_period_frames]
        print(f'[SSVEP] {len(self.sti_lst)} targets, refresh={self.stim_refresh_hz:.0f}Hz')
        
        # 40 个块对应 40 条命令（编号 B01~B40）
        self.commands = [f"B{i+1:02d}" for i in range(40)]
        # 前 5 个块映射到小车命令
        self._car_command_blocks = {
            0: "\u524d\u8fdb",   # 前进
            1: "\u540e\u9000",   # 后退
            2: "\u5de6\u8f6c",   # 左转
            3: "\u505c\u6b62",   # 停止
        ...
        self._hybrid_decoder = None  # 200-target hybrid decoder (lazy init)
        self._spatial_decoder = None  # Spatial decoder (lazy init)
    ...
    def _initItems(self):
        ...
        self.sti_rects = []  # 保持兼容：旧代码可能遍历 sti_rects
        scene_host = QWidget()
        scene_layout = QVBoxLayout(scene_host)
        scene_layout.setContentsMargins(4, 4, 4, 4)
        scene_layout.setSpacing(0)

        # NEW: 40-block SSVEP grid canvas (replaces old StimulusCanvas)
        self.stimulus_canvas = SSVEPGridCanvas(
            scene_host, target_count=self.stim_target_count
        )
    ...
    def _apply_stimulus_profile_to_rects(self):
        """No-op: SSVEPGridCanvas handles frequency/phase assignment internally."""
        pass
    ...
    def setDefaultColor(self):
        """Stop flicker and reset canvas to black."""
        if hasattr(self, 'stimulus_canvas') and self.stimulus_canvas is not None:
            self.stimulus_canvas.stop_flicker()
    ...
    def _render_stim_by_frame(self, frame_index):
        """SSVEPGridCanvas handles its own frame rendering via internal timer."""
        pass

    def _start_stim_render_timer(self):
        """Start the SSVEP grid canvas flicker."""
        if hasattr(self, "stimulus_canvas") and self.stimulus_canvas is not None:
            self.stimulus_canvas.start_flicker()

    def _stop_stim_render_timer(self):
        """Stop the SSVEP grid canvas flicker."""
        if hasattr(self, "stimulus_canvas") and self.stimulus_canvas is not None:
            self.stimulus_canvas.stop_flicker()

    def _stim_render_tick(self):
        """Tick callback - SSVEPGridCanvas handles render internally."""
        # Canvas manages its own timer; just track if stimulation phases are active
        if not self._stimulus_is_active():
            self._stop_stim_render_timer()

    def _stimulus_is_active(self):
        """Check if stimulus canvas is actively flickering."""
        if hasattr(self, "stimulus_canvas") and self.stimulus_canvas is not None:
            return self.stimulus_canvas.is_flickering
        return (
    ...
    def _get_hybrid_decoder(self) -> HybridDecoder:
        """Lazy-init the 200-target hybrid decoder."""
        if self._hybrid_decoder is None:
            self._hybrid_decoder = HybridDecoder(
                ...
                top_k=3,
                n_harmonics=5,
                data_len_sec=self.model_times_sec,
                delay_sec=self.analysis_delay_sec,
            )
            if self._spatial_mode:
                self._hybrid_decoder.set_mode("hybrid")
            else:
                self._hybrid_decoder.set_mode("freq_only")
        return self._hybrid_decoder
    ...
    def _finish_training_collect(self, success):
        ...
        try:
            savemat(saveFile, {
                ...
                'label_idx': label_idx,
                # ── SSVEP 混合编码元数据 (200-target) ──
                'eeg_data': benchmark_trial_data,          # 主数据字段
                'block_idx': int(label_idx // 5) if self._spatial_mode else int(label_idx),  # 0-39
                'fp_idx': int(label_idx % 5) if self._spatial_mode else 0,                   # 0-4
                'target_id': int(label_idx),               # 0-199 or 0-39
                'freq_hz': float(self.sti_lst[label_idx // 5]) if self._spatial_mode else float(self.sti_lst[label_idx]),
                'phase_rad': float(self.stimulus_canvas.block_phase(label_idx // 5)) if self._spatial_mode and hasattr(self, 'stimulus_canvas') else 0.0,
                'stim_phases_rad': np.array(self.stimulus_canvas.phases if hasattr(self, 'stimulus_canvas') else [], dtype=float),
                'n_fixations': int(SSVEPGridCfg.N_FIXATIONS),
                'spatial_mode': int(self._spatial_mode),
    ...
    def _save_online_cycle_data(self, used_data, cycle_start, results, scores_list=None, offsets=None, final_idx=-1, confidence=0.0):
        ...
        try:
            ...
            savemat(saveFile, {
                ...
                'commands': np.array(self.commands, dtype=object),
                # ── SSVEP 混合编码元数据 ──
                'block_idx': int(final_idx // 5) if self._spatial_mode else int(final_idx),
                'fp_idx': int(final_idx % 5) if self._spatial_mode else 0,
                'target_id': int(final_idx),
                'eeg_data': used_data,
                'stim_phases_rad': np.array(self.stimulus_canvas.phases if hasattr(self, 'stimulus_canvas') else [], dtype=float),
                'spatial_mode': int(self._spatial_mode),
            })
