...
class EegSampleBuffer:
    ...
    Stores trial metadata for 200-target hybrid encoding:
      - block_idx (0-39): which flicker block
      - fp_idx (0-4): which fixation point within the block
      - target_id (0-199): global target ID = block_idx * 5 + fp_idx
      - freq_hz, phase_rad: stimulus encoding parameters
    """
    ...
    def clear(self):
        ...
        self.phase_rad: float = 0.0

    def set_trial_meta(self, block_idx: int = -1, fp_idx: int = 0,
                       freq_hz: float = 0.0, phase_rad: float = 0.0):
        """Set trial metadata for hybrid encoding."""
        self.block_idx = int(block_idx)
        self.fp_idx = int(fp_idx)
        self.target_id = self.block_idx * 5 + self.fp_idx if self.block_idx >= 0 else -1
...
def preprocess_model_input(data):
    """Normalize a single EEG trial to the model input convention."""
    if not isinstance(data, np.ndarray) or data.ndim != 2:
        return None
