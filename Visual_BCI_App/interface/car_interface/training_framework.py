...
# 40-target SSVEP frequencies (8.0 ~ 15.8 Hz, step 0.2 Hz)
FORTY_TARGET_FREQS = np.array([8.0 + i * 0.2 for i in range(40)], dtype=float)
...
def extract_training_sample(mat, required_points, sample_rate_hz):
    """Return a model window from benchmark-style files, with legacy fallback."""
    required_points = int(required_points)
    sample_rate_hz = int(sample_rate_hz)

    trial = mat.get("benchmark_trial_data", None)
    if not isinstance(trial, np.ndarray):
        trial = mat.get("raw_data", None)
    if isinstance(trial, np.ndarray) and trial.ndim == 2 and extract_int(mat.get("benchmark_format_version", -1), -1) >= 1:
        pre_sec = extract_float(mat.get("benchmark_pre_stimulus_sec", BENCHMARK_PRE_STIMULUS_SEC), BENCHMARK_PRE_STIMULUS_SEC)
        delay_sec = extract_float(mat.get("benchmark_visual_delay_sec", BENCHMARK_VISUAL_DELAY_SEC), BENCHMARK_VISUAL_DELAY_SEC)
        onset = int(round((pre_sec + delay_sec) * sample_rate_hz))
        end = onset + required_points
        if trial.shape[-1] >= end:
            return np.asarray(trial[:, onset:end], dtype=float)
        return None

    # New format: eeg_data field (200-target hybrid encoding)
    eeg_data = mat.get("eeg_data", None)
    if isinstance(eeg_data, np.ndarray) and eeg_data.ndim == 2 and eeg_data.shape[-1] >= required_points:
        return np.asarray(eeg_data[:, -required_points:], dtype=float)

    data = mat.get("data", None)
    if isinstance(data, np.ndarray) and data.ndim == 2 and data.shape[-1] >= required_points:
        return np.asarray(data[:, -required_points:], dtype=float)
    return None


def extract_target_id(mat) -> int:
    """Extract global target ID (0-199) from .mat file.
    
    Supports both legacy (label_idx) and new hybrid encoding format
    (block_idx + fp_idx → target_id).
    """
    # New format: explicit target_id
    tid = extract_int(mat.get("target_id", -1), -1)
    if tid >= 0:
        return tid

    # New format: block_idx + fp_idx
    block_idx = extract_int(mat.get("block_idx", -1), -1)
    fp_idx = extract_int(mat.get("fp_idx", 0), 0)
    if block_idx >= 0:
        return block_idx * 5 + fp_idx

    # Legacy format: label_idx (0-4 for old car system, 0-39 for 40-target)
    label_idx = extract_int(mat.get("label_idx", -1), -1)
    return label_idx
...
class CarTrainingFramework:
        ...
        for fp in file_list:
            try:
                ...
                if not _stim_freqs_compatible(mat, self.classifier):
                    continue
                # Use new extract_target_id for 200-class support
                label_idx = extract_target_id(mat)
                if label_idx < 0 or label_idx >= self.classifier.Nf:
                    continue

                sample = extract_training_sample(mat, required_points, self.sample_rate_hz)
                if not isinstance(sample, np.ndarray) or sample.ndim != 2:
                    continue

                pts = int(sample.shape[-1])
    ...
    def fit_tdca_from_files(self, file_list):
        if self.model_name != "TDCA" or not hasattr(self.classifier, "fit"):
            return True, "skip"
    ...
    def train_weights(self, checked_files):
        ...
        if self.model_name == "TDCA" and hasattr(self.classifier, "fit"):
            try:
                self.classifier.fit(np.asarray(matrix["x"], dtype=float), np.asarray(matrix["y"], dtype=int))
            except Exception as exc:
                return WeightTrainingResult(False, f"TDCA拟合失败({str(exc)})")
