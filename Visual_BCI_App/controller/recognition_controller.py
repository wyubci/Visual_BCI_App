...
class Recognition(QObject):
    ...
    def __initModel__(self):
        from config import config
        self.num_channels = len(getattr(config, 'lsl_selected_channels', list(range(22, 31)))) if getattr(config, 'device_type', 'neuro_dance_tcp') == 'lsl' else 8
        self.model = SSVEPNet(num_channels=self.num_channels, T=500, num_classes=38).to(self.device)
    ...
    def load_state_dict(self, user):
        ...
        test_data = np.zeros((self.num_channels, 500))
