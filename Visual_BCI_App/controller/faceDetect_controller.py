...
class FaceDetector(QThread):
    ...
    faceExistedSignal = pyqtSignal(object)
    def __init__(self):
        super(FaceDetector, self).__init__()
        self.__initConstValue__()
        self.set()

    def __initConstValue__(self):
        self.leftHull = []
        self.rightHull = []
        self.lineHead = []
        # 闪烁阈值（秒）
    ...
    def set(self):
        ...
        if self.cap.isOpened():
            ...
            print('警告', '摄像头启动失败，请检查连接是否正常')
            self.cap.release()

        self.detector = dlib.get_frontal_face_detector()
        self.model = dlib.shape_predictor("source/device/shape_predictor_68_face_landmarks.dat")
    ...
    def get_head_pose(self, shape):
        ...
        _, rotation_vec, translation_vec = cv2.solvePnP(self.object_pts, image_pts, self.cam_matrix,
                                                        self.dist_coeffs)
        ...
        while (True):
            ...
            if (len(faces) != 0):
                # for k, d in enumerate(faces):
                    # cv2.rectangle(self.image, (d.left(), d.top()), (d.right(), d.bottom()), (0, 0, 255), 1)
                    # self.rectangle = [d.left(), d.top(), d.right() - d.left(), d.bottom() - d.top()]
                    # 使用预测器得到68点数据的坐标
                    # shape = self.model(self.image, d)
                    # for i in range(68):
                    #     cv2.circle(self.image, (shape.part(i).x, shape.part(i).y), 2, (0, 255, 0), -1, 8)
                        # self.point = [shape.part(i).x, shape.part(i).y]

                    # 将脸部特征信息转换为数组array的格式
                    # shape = face_utils.shape_to_np(shape)
                print('face')
                self.faceExistedSignal.emit(True)
            else:
                self.faceExistedSignal.emit(False)
                print('no face')
            time.sleep(0.01)
