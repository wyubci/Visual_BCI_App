...
def _load_samples(data_dir, use_raw=True, latest_n=None):
    ...
    for fp in files:
        ...
        sample = preprocess_model_input(source)
...
def _eval_tdca_block_loso(samples, labels, stim_sec, freqs, sample_rate, block_size):
    if block_size <= 0:
        return {"method": "TDCA_LOSO", "ok": False, "message": "block_size must be positive."}
    if len(samples) < block_size * 2:
        return {"method": "TDCA_LOSO", "ok": False, "message": "LOSO needs at least two blocks."}
    ...
    preds = np.full(len(samples), -1, dtype=int)
    confs = np.zeros(len(samples), dtype=float)
    details = [None] * len(samples)
    fold_results = []
    blocks = [list(range(i, min(i + block_size, len(samples)))) for i in range(0, len(samples), block_size)]

    for fold_idx, test_indices in enumerate(blocks, start=1):
        train_indices = [i for i in range(len(samples)) if i not in test_indices]
        if len(set(labels[train_indices].tolist())) < len(COMMANDS):
            return {
                "method": "TDCA_LOSO",
                "ok": False,
                "message": f"Fold {fold_idx} training set does not contain all classes.",
            }
        model = TDCA(3, stim_sec, freqs, sample_rate=sample_rate)
        train_x = np.asarray([samples[i][:, :required] for i in train_indices], dtype=float)
        train_y = np.asarray([labels[i] for i in train_indices], dtype=int)
        model.fit(train_x, train_y)

        fold_hits = []
        for i in test_indices:
            pred, scores, conf = model.classify_with_scores(samples[i][:, :required])
            preds[i] = int(pred)
            confs[i] = float(conf)
            hit = int(pred) == int(labels[i])
        ...
        fold_results.append(
            {
                "fold": int(fold_idx),
                "accuracy": float(np.mean(fold_hits) * 100.0),
                "correct": int(np.sum(fold_hits)),
                "total": int(len(fold_hits)),
            }
        )

    result = _result("TDCA_LOSO", labels, preds, confs.tolist(), details)
...
def main():
    ...
    parser.add_argument("--latest-n", type=int, default=None, help="Evaluate only the latest N .mat files in data-dir.")
    parser.add_argument("--tdca-cv", choices=["loo", "loso"], default="loo")
    parser.add_argument("--loso-block-size", type=int, default=len(COMMANDS))
    ...
    bundle = _load_samples(data_dir, use_raw=not args.processed_data, latest_n=args.latest_n)
    ...
    if args.tdca_cv == "loso":
        results.append(_eval_tdca_block_loso(samples, labels, stim_sec, freqs, sample_rate, args.loso_block_size))
    else:
